...
**Criterios:**
- En `logout()`, recibir `refreshToken` (opcional, puede venir en cookie o body)
- Si se proporciona token específico, revocar solo ese
- Si no se proporciona, revocar todos los tokens activos del usuario
...
- **Archivos Afectados:**
  ...
  - `apps/api/src/config/env.validation.ts` - AGREGAR validación de `JWT_REFRESH_SECRET`
  - `apps/api/src/modules/auth/auth.module.ts` - AGREGAR ScheduleModule si no existe
