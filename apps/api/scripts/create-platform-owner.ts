...
async function createPlatformOwner() {
  ...
  if (!email || !password) {
    console.error('❌ Error: PLATFORM_OWNER_EMAIL y PLATFORM_OWNER_PASSWORD deben estar configurados como variables de entorno');
    console.error('   Ejemplo: PLATFORM_OWNER_EMAIL=owner@example.com PLATFORM_OWNER_PASSWORD=securepassword npm run script:create-platform-owner');
    process.exit(1);
  }
  ...
  try {
    ...
    if (existingUser) {
      console.log(`⚠️  Usuario ${email} ya existe. Actualizando platformRole...`);
      
      // Actualizar el usuario existente con platformRole
      const updatedUser = await prisma.user.update({
        where: { email },
        data: {
          platformRole: $Enums.platform_role.PLATFORM_OWNER,
          updatedAt: new Date(),
        },
      });

      console.log(`✅ Usuario ${email} actualizado con platformRole = PLATFORM_OWNER`);
    ...
    console.log(`✅ Usuario ${email} creado exitosamente con platformRole = PLATFORM_OWNER`);
    console.log(`\n📋 Detalles del usuario:`);
