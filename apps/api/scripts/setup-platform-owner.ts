/**
 * Script para configurar un usuario como PLATFORM_OWNER
 * 
 * Uso:
 *   npx ts-node apps/api/scripts/setup-platform-owner.ts <email>
 * 
 * Ejemplo:
 *   npx ts-node apps/api/scripts/setup-platform-owner.ts admin@example.com
 */
...
async function setupPlatformOwner(email: string) {
  try {
...
if (!email) {
  ...
  console.log('\nUso:');
  console.log('  npx ts-node apps/api/scripts/setup-platform-owner.ts <email>');
  console.log('\nEjemplo:');
  console.log('  npx ts-node apps/api/scripts/setup-platform-owner.ts admin@example.com');
  process.exit(1);
}
